...
def agent_loop(messages: list):
    while True:
        ...
        parts_with_function_calls = [p for p in parts if p.function_call is not None] # 对于每一个 part，如果有工具调用，那么收集对应的part
        if not parts_with_function_calls:
            return
        result_parts = []
        for p in parts_with_function_calls:
            fc = p.function_call
