...
def make_identity_block(name: str, role: str, team_name: str) -> dict:
    return {
        "role": "user",
        "parts": [{"text": f"<identity>You are '{name}', role: {role}, team: {team_name}. Continue your work.</identity>"}],
    }
            ...
            for _ in range(polls):
                ...
                if unclaimed:
                    ...
                    if len(messages) <= 3:
                        ...
                        messages.insert(1, {"role": "model", "parts": [types.Part(text=f"I am {name}. Continuing.")]})
                    messages.append({"role": "user", "parts": [{"text": task_prompt}]})
                    messages.append({"role": "model", "parts": [types.Part(text=f"Claimed task #{task['id']}. Working on it.")]})
                    resume = True
                    break
